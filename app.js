...
function paymentMatchesFilters(x,city){let p=selectedPeriod();return (!city||x.city===city)&&(!p.year||String(x.year)===String(p.year))&&(!selectedMonths.length||selectedMonths.includes(Number(x.month)))&&(!p.quin||String(x.quin)===String(p.quin))}
