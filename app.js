...
window.W2OpenPayment=openPayment; window.W2ClosePayment=closePaymentModal;
...
async function initApp(){
  try{
    ...
    document.addEventListener('click',e=>{let b=e.target.closest('.pay-btn');if(b&&!b.disabled){e.preventDefault();e.stopPropagation();return openPayment(decodeURIComponent(b.dataset.city));}if(e.target.closest('#closePaymentModal'))return closePaymentModal();if(e.target.closest('#savePayment')){e.preventDefault();return registerPayment();}if(e.target.closest('#payFull'))return $('#mAmount').value=Number($('#mDebt').textContent.replace(/[^0-9,]/g,'').replace(',','.')).toFixed(2);if(e.target.closest('#paymentHistoryBtn')){let h=$('#paymentHistory');h.style.display=h.style.display==='none'?'block':'none';renderPaymentHistory()}});
    $('#paymentModal')?.addEventListener('click',e=>{if(e.target===$('#paymentModal'))closePaymentModal()});
    populateFilters();render();renderHistory();
    console.info(`W2 V2.4.3 iniciado: ${data.length} AWBs carregados do Supabase.`);
